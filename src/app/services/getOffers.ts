...
export async function getInfoJobsOffers() {
  const res = await fetch(
    'https://api.infojobs.net/api/7/offer?category=informatica-telecomunicaciones&category=programacion',
    {
      headers: {
  ...
  const listOfOffers = items.map(item => {
    const { id, title, province, experienceMin, link, teleworking, salaryDescription } = item
